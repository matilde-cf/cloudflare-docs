...
2. Go to **Devices** > **Configuration Policies** > select **New**.
   ...
   ![Create a new configuration profile in Jamf dashboard](~/assets/images/cloudflare-one/connections/jamf-configuration-profiles.png)
...
3. Under **Options**, select **VPN**. Give the VPN a **Connection Name**, select *Per-App VPN* from the **VPN Type** dropdown menu, and check the box for **Automatically start Per-App VPN connection**.
   ...
   ![Give the VPN a Connection Name and continue configuration](~/assets/images/cloudflare-one/connections/connection-name.png)
...
4. Under Per-App VPN Connection Type, set the **Connection Type** to _Custom SSL_ via the dropdown menu and enter `com.cloudflare.cloudflareoneagent` as the **Identifier**,  `1.1.1.1` as the **Server**, and `com.cloudflare.cloudflareoneagent.worker` as the **Provider Bundle Identifier**.
   ...
   ![Choose a connection type and continue configuration](~/assets/images/cloudflare-one/connections/connection-type.png)

5. Set the **Provider Type** to _Packet-Tunnel_ and select the checkboxes for **Include All Networks** and **Enable VPN on Demand**.

   ![Choose the provider type and continue configuration](~/assets/images/cloudflare-one/connections/provider-type.png)
...
8. Go to **Devices*** > **Mobile Device Apps** > select **+ New**.
   ...
   ![Mobile device apps dashboard](~/assets/images/cloudflare-one/connections/mobile-device-apps.png)

9. For **App Type**, select **App Store app or apps purchased in volume** and select **Next**.

   ![Choose app type](~/assets/images/cloudflare-one/connections/choose-app-type.png)

10. In the search bar, enter the name of the app that you want to use the VPN for and select **Next**. The following example uses the Microsoft Edge browser to send traffic through the VPN.

      ![Microsoft Edge example](~/assets/images/cloudflare-one/connections/microsoft-edge-example.png)
...
11. Find the app you are looking for in the search results and select **Add**.
      ...
      ![Find your desired app](~/assets/images/cloudflare-one/connections/add-desired-app.png)

12. Select your preferred **Distribution Method** and under **Per-App Networking**, select the VPN connection you just configured.

      ![Choose your VPN connection under Per-App Networking](~/assets/images/cloudflare-one/connections/per-app-networking.png)
